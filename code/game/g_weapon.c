...
if(g_building.integer){
if(ent->gmodtool){
	VectorMA (muzzle, 800, forward, end);
...
if(!ent->gmodtool){
	...
	if(g_instantgib.integer)
		...
		damage = g_gdamage.integer * 2.5 * s_quadFactor;
	G_Damage( traceEnt, ent, ent, forward, tr.endpos,
		damage, 0, MOD_GAUNTLET );
}
...
if(!ent->gmodtool){
	...
	if(g_instantgib.integer)
		...
		damage = g_gdamage.integer * 2 * s_quadFactor;
	G_Damage( traceEnt, ent, ent, forward, tr.endpos,
		damage, 0, MOD_GAUNTLET );
}
...
if(ent->weaponpack == 4){
	...
	CalcMuzzlePoint ( ent, forward, right, up, muzzle );
if(!g_building.integer){
	VectorMA (muzzle, g_grange.integer*2, forward, end);
	trap_Trace (&tr, muzzle, NULL, NULL, end, ent->s.number, MASK_SHOT);
}
if(g_building.integer){
if(ent->gmodtool){
...
void BFG_Fire4 ( gentity_t *ent ) {
	gentity_t	*m;

	m = fire_bfg4 (ent, muzzle, forward);
	m->damage *= s_quadFactor;
	m->splashDamage *= s_quadFactor;
...
void weapon_new_fire (gentity_t *ent) {
	gentity_t	*m;
...
void weapon_new_fire2 (gentity_t *ent) {
	gentity_t	*m;
	...
	m = fire_new2 (ent, muzzle, forward);
	m->damage *= s_quadFactor;
	m->splashDamage *= s_quadFactor;
...
void weapon_new_fire3 (gentity_t *ent) {
	gentity_t	*m;
	...
	m = fire_new3 (ent, muzzle, forward);
	m->damage *= s_quadFactor;
	m->splashDamage *= s_quadFactor;
...
void weapon_new_fire4 (gentity_t *ent) {
	gentity_t	*m;
	...
	m = fire_new4 (ent, muzzle, forward);
	m->damage *= s_quadFactor;
	m->splashDamage *= s_quadFactor;
...
void weapon_new_fire5 (gentity_t *ent) {
	gentity_t	*m;
	...
	m = fire_new5 (ent, muzzle, forward);
	m->damage *= s_quadFactor;
	m->splashDamage *= s_quadFactor;
...
void weapon_grenadelauncher2_fire (gentity_t *ent) {
	gentity_t	*m;
	...
	m = fire_grenade2 (ent, muzzle, forward);
	m->damage *= s_quadFactor;
	m->splashDamage *= s_quadFactor;
...
void weapon_grenadelauncher3_fire (gentity_t *ent) {
	gentity_t	*m;
	...
	m = fire_grenade3 (ent, muzzle, forward);
	m->damage *= s_quadFactor;
	m->splashDamage *= s_quadFactor;
...
void weapon_grenadelauncher4_fire (gentity_t *ent) {
	gentity_t	*m;
	...
	m = fire_grenade4 (ent, muzzle, forward);
	m->damage *= s_quadFactor;
	m->splashDamage *= s_quadFactor;
...
void Weapon_RocketLauncher2_Fire (gentity_t *ent) {
	gentity_t	*m;

	m = fire_rocket2 (ent, muzzle, forward);
	m->damage *= s_quadFactor;
	m->splashDamage *= s_quadFactor;
...
void Weapon_RocketLauncher3_Fire (gentity_t *ent) {
	gentity_t	*m;
	int			count;

	for( count = 0; count < 3; count++ ) {
		m = fire_rocket3 (ent, muzzle, forward, right, up );
		m->damage *= s_quadFactor;
		m->splashDamage *= s_quadFactor;
	}
...
void Weapon_RocketLauncher4_Fire (gentity_t *ent) {
	gentity_t	*m;

	m = fire_rocket4 (ent, muzzle, forward);
	m->damage *= s_quadFactor;
	m->splashDamage *= s_quadFactor;
...
void Weapon_Plasmagun_Fire3 (gentity_t *ent) {
	gentity_t	*m;

	m = fire_plasma3 (ent, muzzle, forward);
	m->damage *= s_quadFactor;
	m->splashDamage *= s_quadFactor;
...
void Weapon_Plasmagun_Fire4 (gentity_t *ent) {
	gentity_t	*m;

	m = fire_plasma4 (ent, muzzle, forward);
	m->damage *= s_quadFactor;
	m->splashDamage *= s_quadFactor;
...
void weapon_railgun_fire2 (gentity_t *ent) {
	...
	damage = g_rgdamage.integer * 0.5 * s_quadFactor;
	if(g_instantgib.integer)
		damage = 800;
...
void weapon_railgun_fire3 (gentity_t *ent) {
	vec3_t		end;
	vec3_t impactpoint, bouncedir;
	trace_t		trace;
	gentity_t	*tent;
...
void weapon_railgun_fire4 (gentity_t *ent) {
	vec3_t		end;
	vec3_t impactpoint, bouncedir;
	trace_t		trace;
	gentity_t	*tent;
...
//unlagged - backward reconciliation #2
	...
	passent = ent->s.number;
	do {
		trap_Trace (&trace, muzzle, NULL, NULL, end, passent, MASK_NOCSHOT );
		if ( trace.entityNum >= ENTITYNUM_MAX_NORMAL ) {
			break;
		}
		traceEnt = &g_entities[ trace.entityNum ];
...
void Weapon_LightningFire3( gentity_t *ent ) {
	trace_t		tr;
	vec3_t		end;
	vec3_t impactpoint, bouncedir;
	gentity_t	*traceEnt, *tent;
...
void Weapon_Nailgun_Fires (gentity_t *ent) {
	gentity_t	*m;
	int			count;

	for( count = 0; count < NUM_NAILSHOTS; count++ ) {
		m = fire_nails (ent, muzzle, forward, right, up );
		m->damage *= s_quadFactor;
		m->splashDamage *= s_quadFactor;
	}
...
void Weapon_Nailgun_Fire2 (gentity_t *ent) {
	gentity_t	*m;
	int			count;

	for( count = 0; count < 1; count++ ) {
		m = fire_nail2 (ent, muzzle, forward, right, up );
		m->damage *= s_quadFactor;
		m->splashDamage *= s_quadFactor;
	}
...
void Weapon_Nailgun_Fire3 (gentity_t *ent) {
	gentity_t	*m;
	int			count;

	for( count = 0; count < NUM_NAILSHOTS; count++ ) {
		m = fire_nail3 (ent, muzzle, forward, right, up );
		m->damage *= s_quadFactor;
		m->splashDamage *= s_quadFactor;
	}
...
void Weapon_Nailgun_Fire4 (gentity_t *ent) {
	gentity_t	*m;
	int			count;

	for( count = 0; count < NUM_NAILSHOTS; count++ ) {
		m = fire_nail4 (ent, muzzle, forward, right, up );
		m->damage *= s_quadFactor;
		m->splashDamage *= s_quadFactor;
	}
...
void Weapon_Shotgun2_Fire (gentity_t *ent) {
	gentity_t	*m;
	int			count;

	for( count = 0; count < g_sgcount.integer; count++ ) {
		m = fire_shotgun2 (ent, muzzle, forward, right, up );
		m->damage *= s_quadFactor;
		m->splashDamage *= s_quadFactor;
	}
...
void Weapon_Shotgun3_Fire (gentity_t *ent) {
	gentity_t	*m;
	int			count;

	for( count = 0; count < g_sgcount.integer; count++ ) {
		m = fire_shotgun3 (ent, muzzle, forward, right, up );
		m->damage *= s_quadFactor;
		m->splashDamage *= s_quadFactor;
	}
...
void Weapon_Shotgun4_Fire (gentity_t *ent) {
	gentity_t	*m;
	int			count;

	for( count = 0; count < g_sgcount.integer * 1.5; count++ ) {
		m = fire_shotgun4 (ent, muzzle, forward, right, up );
		m->damage *= s_quadFactor;
		m->splashDamage *= s_quadFactor;
	}
...
void Weapon_Grapple2_Fire (gentity_t *ent) {
	gentity_t	*m;
	int			count;

	for( count = 0; count < g_sgcount.integer*0.5; count++ ) {
		m = fire_grapple2 (ent, muzzle, forward, right, up );
		m->damage *= s_quadFactor;
		m->splashDamage *= s_quadFactor;
	}
...
void Weapon_Prox2_Fire (gentity_t *ent) {
	gentity_t	*m;
	int			count;

	for( count = 0; count < g_sgcount.integer*2; count++ ) {
		m = fire_grapple2 (ent, muzzle, forward, right, up );
		m->damage *= s_quadFactor;
		m->splashDamage *= s_quadFactor;
	}
...
void Weapon_Grapple3_Fire (gentity_t *ent) {
	gentity_t	*m;
	int			count;

	for( count = 0; count < 1; count++ ) {
		m = fire_grapple3 (ent, muzzle, forward, right, up );
		m->damage *= s_quadFactor;
		m->splashDamage *= s_quadFactor;
	}
...
void Weapon_Grapple4_Fire (gentity_t *ent) {
	gentity_t	*m;

	if (!ent->client->fireHeld && !ent->client->hook)
		fire_grapple4 (ent, muzzle, forward);

	ent->client->fireHeld = qtrue;
}

void Weapon_Machine3_Fire (gentity_t *ent) {
	gentity_t	*m;
	int			count;

	for( count = 0; count < 1; count++ ) {
		m = fire_machine3 (ent, muzzle, forward, right, up );
		m->damage *= s_quadFactor;
		m->splashDamage *= s_quadFactor;
	}
...
void Weapon_Machine4_Fire (gentity_t *ent) {
	gentity_t	*m;
	gentity_t	*y;
	int			count;
	...
	for( count = 0; count < 1; count++ ) {
		m = fire_machine3 (ent, muzzle, forward, right, up );
		y = fire_plasma (ent, muzzle, forward);
		m->damage *= s_quadFactor;
		m->splashDamage *= s_quadFactor;
		y->damage *= s_quadFactor;
		y->splashDamage *= s_quadFactor;
	}
...
void Weapon_Flamethrower_Fire2 (gentity_t *ent) {
	gentity_t	*m;

	m = fire_flame2 (ent, muzzle, forward);
	m->damage *= s_quadFactor;
	m->splashDamage *= s_quadFactor;
...
void Weapon_Flamethrower_Fire3 (gentity_t *ent) {
	gentity_t	*m;

	m = fire_flame3 (ent, muzzle, forward);
	m->damage *= s_quadFactor;
	m->splashDamage *= s_quadFactor;
...
void Weapon_Flamethrower_Fire4 (gentity_t *ent) {
	gentity_t	*m;

	m = fire_flame4 (ent, muzzle, forward);
	m->damage *= s_quadFactor;
	m->splashDamage *= s_quadFactor;
...
if(ent->weaponpack == 1){
	...
	case 16:					//look for this to add new ones - SWEP_HYPER
		...
		break;
	case 17:
		weapon_new_fire2(ent);
		break;
	case 18:
		weapon_new_fire3(ent);
		break;
	case 19:
		weapon_new_fire4(ent);
		break;
	case 20:
		weapon_new_fire5(ent);
		break;
	default:
// FIXME		G_Error( "Bad ent->s.weapon" );
		break;
...
if(ent->weaponpack == 2){
	...
	case WP_LIGHTNING:
		...
		break;
	case WP_SHOTGUN:
		Weapon_Shotgun2_Fire( ent );
		break;
	case WP_MACHINEGUN:
		Weapon_Machine3_Fire( ent );
		break;
	case WP_GRENADE_LAUNCHER:
		weapon_grenadelauncher2_fire (ent);
		break;
	case WP_ROCKET_LAUNCHER:
		Weapon_RocketLauncher2_Fire( ent );
		break;
	case WP_PLASMAGUN:
	...
	case WP_BFG:
		...
		break;
	case WP_GRAPPLING_HOOK:
		Weapon_Grapple2_Fire( ent );
		break;
	case WP_NAILGUN:
		Weapon_Nailgun_Fire2( ent );
		break;
	case WP_PROX_LAUNCHER:
		Weapon_Prox2_Fire( ent );
		break;
	case WP_CHAINGUN:
		weapon_grenadelauncher_fire (ent);
		weapon_grenadelauncher_fire (ent);
...
if(ent->weaponpack == 3){
	...
	case WP_LIGHTNING:
		...
		break;
	case WP_SHOTGUN:
		Weapon_Shotgun3_Fire( ent );
		break;
	case WP_MACHINEGUN:
		Weapon_Machine4_Fire( ent );
		break;
	case WP_GRENADE_LAUNCHER:
		weapon_grenadelauncher3_fire (ent);
		break;
	case WP_ROCKET_LAUNCHER:
		Weapon_RocketLauncher3_Fire( ent );
		break;
	case WP_PLASMAGUN:
		Weapon_LightningFire( ent );
		Weapon_LightningFire( ent );
	...
	case WP_BFG:
		...
		break;
	case WP_GRAPPLING_HOOK:
		Weapon_Grapple3_Fire( ent );
		break;
	case WP_NAILGUN:
		Weapon_Nailgun_Fire3( ent );
		break;
	case WP_PROX_LAUNCHER:
		BFG_Fire( ent );
		break;
	case WP_CHAINGUN:
		...
		break;
	case WP_FLAMETHROWER:
		Weapon_Flamethrower_Fire2( ent );
		break;
	case WP_ANTIMATTER:
		Bullet_Fire( ent, mod_mgspread, MACHINEGUN_DAMAGE*10 );
		break;
	default:
// FIXME		G_Error( "Bad ent->s.weapon" );
		break;
...
if(ent->weaponpack == 4){
	...
	case WP_LIGHTNING:
		Weapon_RocketLauncher2_Fire( ent );
		weapon_grenadelauncher4_fire (ent);
		break;
	case WP_SHOTGUN:
		Weapon_Shotgun4_Fire( ent );
		break;
	case WP_MACHINEGUN:
			Bullet_Fire( ent, mod_mgspread, MACHINEGUN_DAMAGE*2 );
		break;
	case WP_GRENADE_LAUNCHER:
		weapon_grenadelauncher4_fire (ent);
		break;
	case WP_ROCKET_LAUNCHER:
		Weapon_RocketLauncher4_Fire( ent );
		break;
	case WP_PLASMAGUN:
		Weapon_Plasmagun_Fire4( ent );
		break;
	case WP_RAILGUN:
		weapon_railgun_fire4( ent );
		break;
	case WP_BFG:
		BFG_Fire4( ent );
		break;
	case WP_GRAPPLING_HOOK:
		Weapon_Grapple4_Fire( ent );
		break;
	case WP_NAILGUN:
		Weapon_Nailgun_Fire4( ent );
		break;
	case WP_PROX_LAUNCHER:
		weapon_proxlauncher_fire( ent );
		weapon_proxlauncher_fire( ent );
		break;
	case WP_CHAINGUN:
		Bullet_Fire( ent, mod_cgspread, CHAINGUN_DAMAGE*1.5 );
		break;
	case WP_FLAMETHROWER:
		Weapon_Flamethrower_Fire3( ent );
		break;
	case WP_ANTIMATTER:
		Weapon_Nailgun_Fires( ent );
		break;
	default:
// FIXME		G_Error( "Bad ent->s.weapon" );
		break;
