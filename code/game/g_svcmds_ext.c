...
void Svcmd_QvmWrite_f( void )
{
	...
	G_Printf( "mode list:\n" );
	G_Printf( "<level> | Editing level variables\n" );
	G_Printf( "<entity> | Editing entity variables\n" );
    return;}
	if( trap_Argc( ) == 2 ){
	if(!Q_stricmp (mode, "level")){
    G_Printf( "usage: qvmwrite <mode> <variable> <value>\n" );
	G_Printf( "<variable> | id variable\n" );
	G_Printf( "<value> | any type\n" );
	}
	if(!Q_stricmp (mode, "entity")){
    G_Printf( "usage: qvmwrite <mode> <variable> <value> <field> <value2>\n" );
	G_Printf( "<variable> | id variable\n" );
	G_Printf( "<value> | any type\n" );
	G_Printf( "<field> | entity field\n" );
	G_Printf( "<value2> | any type\n" );
	}
    return;}
  ...
  trap_Argv( 5, value4, sizeof( value4 ) );
 

		if(!Q_stricmp (mode, "level")){
			G_QvmWrite_LevelVariable(value1, value2);
		}		
		if(!Q_stricmp (mode, "entity")){
		entscr = NULL;
		if(!Q_stricmp (value3, "classname")){
			while ( (entscr = G_ScrFind(entscr, FOFS(classname), G_ArenaScriptAutoChar( value4 ), "=")) != NULL ) {
			G_QvmWrite_EntityVariable(value1, value2, entscr);}	
		}
		if(!Q_stricmp (value3, "playername")){
			while ( (entscr = G_ScrFind(entscr, FOFS(playername), G_ArenaScriptAutoChar( value4 ), "=")) != NULL ) {
			G_QvmWrite_EntityVariable(value1, value2, entscr);}	
		}
		if(!Q_stricmp (value3, "target")){
			while ( (entscr = G_ScrFind(entscr, FOFS(target), G_ArenaScriptAutoChar( value4 ), "=")) != NULL ) {
			G_QvmWrite_EntityVariable(value1, value2, entscr);}	
		}
		if(!Q_stricmp (value3, "target2")){
			while ( (entscr = G_ScrFind(entscr, FOFS(target2), G_ArenaScriptAutoChar( value4 ), "=")) != NULL ) {
			G_QvmWrite_EntityVariable(value1, value2, entscr);}	
		}
		if(!Q_stricmp (value3, "targetname")){
			while ( (entscr = G_ScrFind(entscr, FOFS(targetname), G_ArenaScriptAutoChar( value4 ), "=")) != NULL ) {
			G_QvmWrite_EntityVariable(value1, value2, entscr);}	
		}
		if(!Q_stricmp (value3, "targetname2")){
			while ( (entscr = G_ScrFind(entscr, FOFS(targetname2), G_ArenaScriptAutoChar( value4 ), "=")) != NULL ) {
			G_QvmWrite_EntityVariable(value1, value2, entscr);}	
		}
		}
}
...
void Svcmd_QvmRead_f( void )
{
	...
	G_Printf( "mode list:\n" );
	G_Printf( "<level> | Read level variables\n" );
	G_Printf( "<entity> | Read entity variables\n" );
    return;}
	if( trap_Argc( ) == 2 ){
	if(!Q_stricmp (mode, "level")){
    G_Printf( "usage: qvmread <mode> <variable> <destcvar>\n" );
	G_Printf( "<variable> | id variable\n" );
	G_Printf( "<destcvar> | any cvar\n" );
	}
	if(!Q_stricmp (mode, "entity")){
    G_Printf( "usage: qvmread <mode> <variable> <destcvar> <field> <value2>\n" );
	G_Printf( "<variable> | id variable\n" );
	G_Printf( "<destcvar> | any type\n" );
	G_Printf( "<field> | entity field\n" );
	G_Printf( "<value2> | any type\n" );
	}
    return;}
  ...
  trap_Argv( 5, value4, sizeof( value4 ) );
		...
		if(!Q_stricmp (mode, "level")){
			trap_Cvar_Set( value2, va("%s", G_QvmRead_LevelVariable(value1)) );
		}		
		if(!Q_stricmp (mode, "entity")){
		entscr = NULL;
		if(!Q_stricmp (value3, "classname")){
			while ( (entscr = G_ScrFind(entscr, FOFS(classname), G_ArenaScriptAutoChar( value4 ), "=")) != NULL ) {
			trap_Cvar_Set( value2, va("%s", G_QvmRead_EntityVariable(value1, entscr)) );}	
		}
		if(!Q_stricmp (value3, "playername")){
			while ( (entscr = G_ScrFind(entscr, FOFS(playername), G_ArenaScriptAutoChar( value4 ), "=")) != NULL ) {
			trap_Cvar_Set( value2, va("%s", G_QvmRead_EntityVariable(value1, entscr)) );}	
