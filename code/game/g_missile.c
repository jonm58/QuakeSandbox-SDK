...
=================
*/
gentity_t *fire_new (gentity_t *self, vec3_t start, vec3_t dir) {
	gentity_t	*bolt;
...
=================
*/
gentity_t *fire_new2 (gentity_t *self, vec3_t start, vec3_t dir) {
	gentity_t	*bolt;
...
=================
*/
gentity_t *fire_new3 (gentity_t *self, vec3_t start, vec3_t dir) {
	gentity_t	*bolt;
...
=================
*/
gentity_t *fire_new4 (gentity_t *self, vec3_t start, vec3_t dir) {
	gentity_t	*bolt;
...
=================
*/
gentity_t *fire_new5 (gentity_t *self, vec3_t start, vec3_t dir) {
	gentity_t	*bolt;
...
//unlagged - projectile nudge
	...
	bolt->splashMethodOfDeath = MOD_GAUNTLET;
	bolt->methodOfDeath = MOD_GAUNTLET;
	bolt->clipmask = MASK_SHOT;
	bolt->target_ent = NULL;
	bolt->s.pos.trType = TR_GRAVITY;
	...
	r = random() * M_PI * 2.0f;
	u = sin(r) * crandom() * 1200 * 16;
	r = cos(r) * crandom() * 1200 * 16;
...
//unlagged - projectile nudge
	...
	r = random() * M_PI * 2.0f;
	u = sin(r) * crandom() * 10200 * 16;
	r = cos(r) * crandom() * 10200 * 16;
	VectorMA( start, 8192 * 16, forward, end);
	VectorMA (end, r, right, end);
	VectorMA (end, u, up, end);
	VectorSubtract( end, start, dir );
...
//unlagged - projectile nudge
	...
	bolt->s.pos.trTime = level.time - MISSILE_PRESTEP_TIME;		// move a bit on the very first frame
	VectorCopy( start, bolt->s.pos.trBase );
	VectorScale( dir, g_ftspeed.value*1.5, bolt->s.pos.trDelta );
	SnapVector( bolt->s.pos.trDelta );			// save net bandwidth
	VectorCopy (start, bolt->r.currentOrigin);
